...
func TestDynamoPaper2(t *testing.T){
    t.Logf("DynamoPaper test")
    cmd := InitDynamoServer("./threeserver.ini")
    ready := make(chan bool)
    go StartDynamoServer(cmd, ready)
    defer KillDynamoServer(cmd)
    ...
    if gotValuePtr == nil {
        t.Fail()
        t.Logf("TestDynamoPaper: Failed to get first value 1")
    }
    gotValue = *gotValuePtr
    if(len(gotValue.EntryList) != 1 || !valuesEqual(gotValue.EntryList[0].Value, []byte("abcde"))){
        t.Fail()
        t.Logf("TestDynamoPaper: First value doesn't match 1")
    }
    gotValuePtr = clientInstance2.Get("s1")
    if gotValuePtr == nil {
        t.Fail()
        t.Logf("TestDynamoPaper: Failed to get first value 2")
    }
    gotValue = *gotValuePtr
    if(len(gotValue.EntryList) != 1 || !valuesEqual(gotValue.EntryList[0].Value, []byte("abcde"))){
        t.Fail()
        t.Logf("TestDynamoPaper: First value doesn't match 2")
    }
    clientInstance0.Put(mydynamo.NewPutArgs("s1", gotValue.EntryList[0].Context, []byte("bcdef")))
    gotValuePtr = clientInstance0.Get("s1")
    if gotValuePtr == nil {
    ...
    if gotValuePtr == nil {
        t.Fail()
        t.Logf("TestDynamoPaper: Failed to get second value 1")
    }
    gotValue = *gotValuePtr
    if(len(gotValue.EntryList) != 1 || !valuesEqual(gotValue.EntryList[0].Value, []byte("bcdef"))){
        t.Fail()
        t.Logf("TestDynamoPaper: Second value doesn't match 1")
    }
    gotValuePtr = clientInstance2.Get("s1")
    if gotValuePtr == nil {
        t.Fail()
        t.Logf("TestDynamoPaper: Failed to get second value 2")
    }
    gotValue = *gotValuePtr
    if(len(gotValue.EntryList) != 1 || !valuesEqual(gotValue.EntryList[0].Value, []byte("bcdef"))){
        t.Fail()
        t.Logf("TestDynamoPaper: Second value doesn't match 2")
    }
/*
    clientInstance0.Gossip()
    clientInstance1.Put(mydynamo.NewPutArgs("s1", gotValue.EntryList[0].Context, []byte("cdefg")))
